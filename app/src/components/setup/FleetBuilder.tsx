  ...
  const handleFinish = () => {
    ...
    for (let idx = 0; idx < totalPlayersCount; idx++) {
      ...
      finalShips.push({
        ...
        equippedWeapons: Array.from({ length: draftChassis.weaponSlots }).map((_, i) => draft.weapons[i] || null),
        equippedSubsystems: Array.from({ length: draftChassis.internalSlots }).map((_, i) => draft.subsystems[i] || null),
        criticalDamage: [],
        scars: [],
        ...
        {step === 3 && activeChassis && (
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'auto' }}>
              ...
              <SmartTooltip 
                content={!allReady ? "Not all ships are ready" : undefined} 
                as="button"
