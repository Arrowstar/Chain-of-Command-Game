...
export function purchaseOfficerTraining(params: {
  ...
  const tierMap: Record<string, { key: string; toTier: SkillDieTier }> = {
    ...
    elite: { key: 'elite-to-legendary', toTier: 'legendary' },
  };
...
export function applyEliteReward(
  ...
  switch (reward.type) {
    ...
    case 'tech': {
      if (reward.itemId) {
        const specificTech = getTechById(reward.itemId);
        if (specificTech) {
          tech.push(specificTech);
          narrative = `Acquired Experimental Tech: ${specificTech.name}.`;
        } else {
          narrative = `Failed to locate Experimental Tech: ${reward.itemId}.`;
        }
      } else {
